...
const DestinationPage = ({ entireSectionParentElement }) => {
	const [destinationPageDatas, setDestinationPageDatas] = useState({ name: "", images: "", description: "", distance: "", travel: "" });
		...
		<main class="lg:flex lg:w-[90%] lg:justify-between lg:mx-auto lg:gap-[5%]">
			<article class="lg:w-[50%]">
				...
				<div class="w-[60%] h-[220px] tablet:h-[250px] mdHeight:h-[280px] lgHeight:h-[310px] mt-4 lg:mt-16 mx-auto">
					<img class="max-w-[100%] max-h-[100%] mx-auto current-tab-img" src={images.png} alt="destination-img" />
				</div>
			</article>
			<article class="lg:w-[50%] lg:flex lg:flex-col lg:items-start lg:mt-28">
				<DestinationPageTabs handleTabsClick={handleTabsClick} />

				<h1 class="text-center text-[80px] font-Bellefair mt-4 current-tab-title uppercase lg:text-[100px] lg:mt-0">{name}</h1>

				<p class="w-[80%] mx-auto pb-6 border-opacity-50 border-b-2 border-white mb-6 mt-4 text-center text-[16px] leading-[30px] font-Barlow text-primaryColor current-tab-description lg:mt-0 lg:text-left lg:mx-0 lg:text-[18px] tracking-[1px]">{description}</p>
